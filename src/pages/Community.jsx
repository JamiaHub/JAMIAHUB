...
const Community = () => {
  ...
  useEffect(() => {
    const switchChannel = async () => {
      ...
      try {
        console.log(
          "Attempting to switch channel for room ID:",
          activeChatRoom,
        );

        const activeRoom = chatRooms.find(
          (room) => room._id === activeChatRoom,
        );
        ...
        if (!activeRoom.streamChatId) {
          console.error(
            "No streamChatId found for room:",
  ...
  const SocietyCard = ({ society, index }) => (
    <div className="group relative flex-shrink-0 w-72 h-80 cursor-pointer">
      <div
        className={`absolute inset-0 bg-gradient-to-br ${society.color} rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-xl`}
      ></div>

      <div
        className={`relative w-full h-full bg-gradient-to-br ${society.color} backdrop-blur-xl rounded-3xl shadow-2xl border ${society.border} hover:border-opacity-60 transition-all duration-500 overflow-hidden transform hover:scale-105 hover:-translate-y-2`}
      >
          ...
          <div className="w-28 h-28 mb-6 transform transition-all duration-500 group-hover:scale-110 overflow-hidden rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 flex items-center justify-center">
            <img
              ...
              onError={(e) => {
                ...
                e.target.src =
                  "https://via.placeholder.com/200x200/6366f1/ffffff?text=" +
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-gray-900">
                  ...
                  return (
                    ...
                    >
                      <div className="relative h-48 overflow-hidden">
                        ...
                        <div
                          className={`absolute top-4 left-4 px-3 py-1 rounded-full text-xs  uppercase tracking-wide shadow-lg ${
                            isUpcoming
                  ...
                  >
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between text-gray-400 text-sm">
                        <button
                          onClick={(e) =>
                            handleBlogLikeClicked(blog._id || blog.id, e)
                          }
                        ...
                        >
                          {likedBlogs.has(blog._id || blog.id) ? "❤️" : "🤍"}{" "}
                          {blog.likes}
                        </button>
                      </div>
                    </div>
            ...
            <div className="p-8">
              ...
              <div className="flex items-center gap-4 mb-6 pb-6 border-b border-gray-700">
                ...
                <div className="ml-auto flex items-center gap-4">
                  <button
                    onClick={(e) =>
                      handleBlogLikeClicked(
                        selectedBlog._id || selectedBlog.id,
                        e,
                      )
                    }
              ...
              <div className="flex flex-wrap gap-2 pt-6 border-t border-gray-700">
                {JSON.parse(selectedBlog.tags[0]).map((tag, i) => (
                  <span
                    key={i}
                    className="bg-gray-700/50 text-gray-300 px-4 py-2 rounded-full text-sm"
                  >
                    #{tag}
                  </span>
                ))}
      ...
      <button
        onClick={() =>
          isAuthenticated ? setIsChatOpen(true) : setAddbtnClick(true)
        }
      ...
      {activeTab == "blogs" && (
        <button
          onClick={() =>
            isAuthenticated ? setIsBlogOpen(true) : setAddbtnClick(true)
          }
          className="fixed bottom-8 right-24 bg-gradient-to-r from-blue-600 to-cyan-500 text-white p-4 rounded-full shadow-2xl hover:shadow-purple-500/50 transition-all duration-300 z-50 hover:scale-110"
        >
          <Plus className="w-6 h-6" />
            ...
            <div className="w-64 bg-gray-800/50 border-r border-gray-700 p-4 overflow-y-auto">
              ...
              {chatRooms.map((room) => {
                const isMember = room.members?.some(
                  (member) =>
                    member === authUser?._id || member._id === authUser?._id,
                );
                return (
              ...
              <div className="bg-gray-800/50 border-t border-gray-700 p-4">
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 bg-gray-700 text-white px-6 py-2 rounded-lg font-semibold hover:bg-gray-600 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    disabled={submitLoading}
                    className="flex-1 bg-gradient-to-r from-purple-500 to-pink-500 text-white px-6 py-2 rounded-lg font-semibold hover:shadow-lg hover:shadow-purple-500/50 transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {submitLoading ? (
                      <>
                        <svg
